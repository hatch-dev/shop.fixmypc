...
class OrdersController extends ControllerHelper
{
    ...
    public function paymentDone(Request $request)
    {
        try {
            ...
            } else if ((int)$request->order_method == Config::get('constants.paymentMethod.SUMUP')){
                $paymentDone = true;
    ...
    public function action(Request $request, TwilioService $twilio)
    {
        try {
            $lang = $request->header('language');
            ...
            }else if ((int)$payment->sumup != 1 &&
                (int)$request->order_method == Config::get('constants.paymentMethod.SUMUP')
            ) {
                return response()->json(Validation::error($request->token,
                    __('lang.accepted_gateway', [], $lang)
                ));
            ...
            if (count($existingCart) > 0) {
                ...
                if ($request->user('user')) {
                    ...
                    if(isset($request->orderId)){
                        $orderArr['order'] = $request->orderId;
                        $orderArr['trans_id'] = $request->transactionId;
                    }else{
                ...
                foreach ($orderedProducts as $orderedProduct) {
                    ...
                    $inventoryId = $orderedProduct['inventory_id'];
                    $isProcured = Product::where([
                        'id' => $productId,
                        'procurement' => 1
                    ])->exists();
                    if (!$isProcured) {
                    ...
                    $categoryIds = ProductCategory::where('product_id', $productId)
                        ->pluck('category_id')
                        ->toArray();

                    $supplierIds = SupplierCategory::whereIn('category_id', $categoryIds)
                        ->pluck('supplier_id')
                        ->unique()
                        ->toArray();
                ...
                $suppliers = Supplier::whereIn('id', array_values($allSuppliers))->get();

                foreach ($suppliers as $supplier) {
                    $alreadySentForOrder = SupplierNotification::where([
                        'supplier_id' => $supplier->id,
                        'order_id'    => $order->id,
                    ])->exists();

                    if ($alreadySentForOrder) {
                        continue;
                    }
                    ...
                    $link = config('app.url') . '/supplier/bid/' . urlencode($encrypted);
                    $message = "📦 New Fullfilment Request!\n\n"
                    . "Order ID: {$order->order}\n"
                    . "Products: " . count($productMap[$supplier->id]) . "\n"
                    . "Link: {$link}";

                    $fullPhone = $supplier->country_code . $supplier->phone;
                    $twilio->sendWhatsApp($fullPhone, $message);

                    foreach ($productMap[$supplier->id] as $productData) {
                ...
                if ($result) {
                    ...
                    if ((int)$request->order_method == Config::get('constants.paymentMethod.STRIPE')) {
                        ...
                        $re['order_method'] = Config::get('constants.paymentMethod.SUMUP');
                        Order::where('id', $order->id)->update([
                            'total_amount' => $totalPrice - $offeredVoucher,
                        ]);
            ...
            return response()->json(Validation::error($request->token, $e->getMessage(), $e->getLine()));
        }
    }
    ...
    public function sendOrderEmail(Request $request, $id)
    {
        ...
        //   return response()->json($mailDataLang);
