...
class FlashSalesController extends ControllerHelper
{
    ...
    public function action(Request $request, $id = null)
    {
        ...
        if ($request->start_time) {
            $startTime = \DateTime::createFromFormat('d-m-Y H:i:s', $request->start_time);
            if ($startTime) {
                $request['start_time'] = $startTime->format('Y-m-d H:i:s');
            } else {
                // Fallback: try to parse as is if format is different
                $request['start_time'] = date('Y-m-d H:i:s', strtotime($request->start_time));
            }
        }

        if ($request->end_time) {
            $endTime = \DateTime::createFromFormat('d-m-Y H:i:s', $request->end_time);
            if ($endTime) {
                $request['end_time'] = $endTime->format('Y-m-d H:i:s');
            } else {
                // Fallback: try to parse as is if format is different
